...
# Force standard asyncio event loop policy to avoid uvloop patch errors with nest_asyncio
if sys.platform != "win32":
    asyncio.set_event_loop_policy(asyncio.DefaultEventLoopPolicy())

# Monkeypatch langchain_community.chat_models.vertexai to avoid ModuleNotFoundError in ragas
try:
...
# Create and set a standard event loop for the current thread
try:
...
# ─────────────────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Tumor RAG — Eval Suite",
    page_icon="🧬",
    layout="wide",
)
...
# Header
# ─────────────────────────────────────────────────────────────────────────────
st.title("🧬 Tumor & Oncology RAG — Evaluation Suite")
st.caption(
...
with tab1:
    ...
    st.caption(f"✅ {len(rag_rows)} golden RAG samples from 5 oncology domains")
    ...
    st.caption("6 guardrails test cases: 3 adversarial (should block) + 3 legit oncology questions (should pass)")
